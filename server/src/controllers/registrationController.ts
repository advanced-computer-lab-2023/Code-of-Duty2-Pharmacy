...
import { sendAcceptanceEmailToPharmacist, sendRejectionEmailToPharmacist } from "../services/admins";
...
export const registerPatient = async (req: Request, res: Response) => {
  try {
    ...
    if (existingAdmin || existingPharmacist || existingPatient || existingDoctor) {
      return res.status(StatusCodes.BAD_REQUEST).json({ message: "Username already taken by another user !" });
    }

    const existingMail = await Patient.findOne({ email });

    if (existingMail) {
      return res.status(StatusCodes.BAD_REQUEST).json({ message: "Email already taken by another patient !" });
    }
...
export const registerPharmacist = async (req: Request, res: Response) => {
  try {
    ...
    if (existingMail) {
      return res.status(StatusCodes.BAD_REQUEST).json({ message: "Email already taken by another pharmacist !" });
    }
...
export const acceptPharmacistRegistrationRequest = async (req: AuthorizedRequest, res: Response) => {
  const request = await PharmacistRegistrationRequest.findOne({
    username: req.body.username
  });

  if (!request) {
    return res.status(StatusCodes.BAD_REQUEST).json({ message: "pharmacist request not found" });
  }
...
export const rejectPharmacistRegistrationRequest = async (req: AuthorizedRequest, res: Response) => {
