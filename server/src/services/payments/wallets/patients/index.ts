...
export const authenticateWalletPatient = async (patientId: string, walletPinCode: string) =>
...
export const AddPatientAWallet = async (patientId: string, desiredCurrency: string, pinCode: string) => {
  await addWallet(patientId, UserRole.PATIENT, desiredCurrency, pinCode);
};

export const performWalletTransaction = async (patientId: string, transactionAmount: number) => {
  await updatePatientWalletBalance(patientId, -transactionAmount);
};

export const rechargePatientWallet = async (patientId: string, transactionAmount: number) => {
  await updatePatientWalletBalance(patientId, transactionAmount);
};

const updatePatientWalletBalance = async (patientId: string, transactionAmount: number) => {
